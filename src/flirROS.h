...
//#include </opt/ros/fuerte/include/opencv2/highgui.hpp>
...
int trackObject()
{
	...
	video = cvCaptureFromCAM(1);
 	cvSetCaptureProperty(video, CV_CAP_PROP_FRAME_WIDTH, 960);
	cvSetCaptureProperty(video, CV_CAP_PROP_FRAME_HEIGHT, 720);
    ...
    if ( !capture ){
		ROS_ERROR("Cannot open capture device");
        	return -1; 
    }
	...
	/* Create required images once */
 	if( frame_count == 0 )
 	{
		...
		CvSize size;
        	...
        	double frame_rate = 14; //cvGetCaptureProperty(video, CV_CAP_PROP_FPS);

        	webcamWriter = cvCreateVideoWriter("/home/sealab/Videos/cameraOut.avi", CV_FOURCC('D','I','V','X'), frame_rate, size, 1);
		//initialize blobs and flir video writers
		size.width = img->width;
		size.height = img->height;
		
		flirWriter = cvCreateVideoWriter("/home/sealab/Videos/flirOut.avi", CV_FOURCC('D','I','V','X'), frame_rate, size, 1);
		blobsWriter = cvCreateVideoWriter("/home/sealab/Videos/blobsOut.avi", CV_FOURCC('D','I','V','X'), frame_rate, size, 1);
	...
	webcam_img = cvQueryFrame(video);
  	   	...
  	   	/* Always check if the device returns a frame */
  	   	if( !img ){
			ROS_ERROR("Could not get a frame from the capture device");
   			return -1;
		}
